 ...
 * \defgroup onoff IncastApplication
 *
...
*/
class IncastApplication : public Application 
{
public:
  /**
  ...
  /**
   * \brief Set the number of flows to send per incast host.
   *
   * Once the number of flows has been sent, no more will be sent.
   * 
   * \param numFlows the total number of flows to send
   */
  void SetNumFlows (uint32_t numFlows);

  /**
   * \brief Set the size of each incast flow.
   *
   * Incast hosts will send this much data at line rate in a new connection.
   *
   * \param sendSize the size of each incast flow
   */
...
private:
  ...
  /**
   * \brief Disconnect from active socket.
   */
  ...
  uint32_t	  m_flowsSent;    //!< Total number of independent flows sent so far
  EventId         m_startStopEvent;     //!< Event id for next start or stop event
  EventId         m_sendEvent;    //!< Event id of pending "send packet" event
  TypeId          m_tid;          //!< Type of the socket used

  uint32_t	  m_numFlows;	  //!< Number of flows to send per incast host
  uint32_t	  m_sendSize;	  //!< Size of each flow sent by incast host
...
private:
  ...
  /**
   * \brief for socket's SetSendCallback
   */
